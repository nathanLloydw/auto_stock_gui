...
$sql = "SELECT order_reference,t.product_id,product_name,cast(SUM(t.quantity)AS int) AS stock_sold,t.sales_price,cost_price,CAST((SUM(t.quantity)*t.sales_price) - (SUM(t.quantity)*cost_price)AS int) AS product_profit FROM transactions t 
INNER JOIN products AS p ON t.product_id=p.product_id WHERE customer_id not LIKE '1%' AND t.user != 'ORCHARDS' AND t.customer_id != 'ORCHARD' AND t.customer_id != 'ORCHARDV' and t.quantity > 0 AND delivery_date >= '2020-03-27' 
GROUP BY t.product_id,t.sales_price order by (SUM(t.quantity)*t.sales_price) - (SUM(t.quantity)*cost_price) desc";
